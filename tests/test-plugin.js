...
test('the editor receives the styles and the panel logic together', async () => {
    const res = await get('/plugins');
    assert.strictEqual(res.status, 200);
    assert.match(res.body, /red-ui-clipboard-dialog-import-tab-apispec/);
    assert.match(res.body, /#flowgen-op-list/, 'the panel styles are delivered');
    assert.match(res.body, /registerPlugin\('node-red-flowgen'/,
        'the logic is inlined in the markup');
...
test('the editor script is not served as a separate asset', async () => {
    assert.strictEqual((await get('/flowgen/editor.js')).status, 404);
});
...
test('the served flowgen.js works when evaluated as browser code', async () => {
    const vm = require('vm');
    const source = (await get('/flowgen/generator.js')).body;
...
test('everything the browser loads is served by the local admin server', async () => {
    for (const asset of ['generator.js', 'yaml-parser.js']) {
...
test('the served plugin markup carries the panel and its handlers', async () => {
    const res = await get('/plugins');
    assert.strictEqual(res.status, 200);
    assert.match(res.body, /registerPlugin\('node-red-flowgen'/);
    for (const marker of ['flowgen-spec-text', 'flowgen-select-btn', 'flowgen-back-btn',
...
test('the runtime exposes only the documented routes', async () => {
    assert.strictEqual((await get('/flowgen/generator.js')).status, 200);
    assert.strictEqual((await get('/flowgen/yaml-parser.js')).status, 200);
