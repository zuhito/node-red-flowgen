...
| `1password.local/connect/1.5.7/openapi.yaml` | 成功 | openapi 3.0 | 15 | DELETE 1, GET 11, PATCH 1, POST 1, PUT 1 | 3 | `GET /health` | `80668dce` | メソッドが多い、認証なしで呼べるのは 15 中 3 |
| `airbyte.local/config/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 102 | GET 2, POST 100 | 2 | `GET /v1/health` | `80668dce` | 大規模、認証なしで呼べるのは 102 中 2 |
| `akeneo.com/1.0.0/swagger.yaml` | 成功 | swagger 2 | 144 | DELETE 6, GET 75, PATCH 39, POST 23, PUT 1 | 14 | `GET /api/rest/v1` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 144 中 14 |
...
| `apicurio.local/registry/1.3.2.Final/openapi.yaml` | 成功 | openapi 3.0 | 33 | DELETE 6, GET 14, POST 5, PUT 8 | 2 | `GET /artifacts` | `80668dce` | メソッドが多い、認証なしで呼べるのは 33 中 2 |
...
| `apidapp.com/2019-02-14T164701Z/openapi.yaml` | 成功 | openapi 3.0 | 54 | DELETE 1, GET 17, OPTIONS 24, POST 12 | 4 | `GET /erc20` | `80668dce` | メソッドが多い、認証なしで呼べるのは 54 中 4 |
...
| `axesso.de/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 4 | GET 4 | 1 | `GET /amz/sort-options` | `80668dce` | 参照のみ、認証なしで呼べるのは 4 中 1 |
...
| `bbci.co.uk/1.0/openapi.yaml` | 成功 | openapi 3.0 | 30 | GET 30 | 5 | `GET /categories` | `80668dce` | 参照のみ、認証なしで呼べるのは 30 中 5 |
...
| `bigoven.com/partner/openapi.yaml` | 成功 | openapi 3.0 | 66 | DELETE 6, GET 33, POST 17, PUT 10 | 7 | `GET /grocerylist` | `80668dce` | メソッドが多い、認証なしで呼べるのは 66 中 7 |
...
| `bungie.net/2.18.0/openapi.yaml` | 成功 | openapi 3.0 | 134 | GET 88, POST 46 | 13 | `GET /App/FirstParty/` | `80668dce` | 大規模、認証なしで呼べるのは 134 中 13 |
...
| `chaingateway.io/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 119 | DELETE 9, GET 70, POST 40 | 26 | `GET /v2/bitcoin/blocks/number` | `80668dce` | 大規模、認証なしで呼べるのは 119 中 26 |
| `clearblade.com/3.0/swagger.yaml` | 成功 | swagger 2 | 220 | DELETE 34, GET 94, POST 54, PUT 38 | 2 | `GET /admin/database/status` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 220 中 2 |
| `clicksend.com/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 205 | DELETE 15, GET 93, POST 50, PUT 47 | 41 | `GET /account` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 205 中 41 |
| `clickup.com/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 2 | GET 1, POST 1 | 1 | `GET /questions` | `ce61d5ae` | 小規模、認証なしで呼べるのは 2 中 1 |
| `clubhouseapi.com/1/openapi.yaml` | 成功 | openapi 3.0 | 41 | GET 10, POST 31 | 10 | `GET /check_for_update` | `80668dce` | 認証なしで呼べるのは 41 中 10 |
...
| `dnd5eapi.co/0.1/openapi.yaml` | 成功 | openapi 3.0 | 47 | GET 47 | 1 | `GET /api` | `80668dce` | 参照のみ、認証なしで呼べるのは 47 中 1 |
...
| `hackathonwatch.com/0.1/openapi.yaml` | 成功 | openapi 3.0 | 4 | GET 4 | 2 | `GET /hackathons/coming.json` | `80668dce` | 参照のみ、認証なしで呼べるのは 4 中 2 |
...
| `just-eat.co.uk/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 97 | DELETE 4, GET 28, PATCH 2, POST 25, PUT 38 | 1 | `GET /delivery/pools` | `80668dce` | メソッドが多い、認証なしで呼べるのは 97 中 1 |
| `languagetool.org/1.1.2/swagger.yaml` | 成功 | swagger 2 | 5 | GET 2, POST 3 | 1 | `GET /languages` | `ce61d5ae` | 認証なしで呼べるのは 5 中 1 |
| `learnifier.com/1.1.0/swagger.yaml` | 成功 | swagger 2 | 34 | DELETE 3, GET 20, PATCH 3, POST 8 | 4 | `GET /coursedesigns` | `80668dce` | メソッドが多い、認証なしで呼べるのは 34 中 4 |
...
| `magick.nu/1.0/swagger.yaml` | 成功 | swagger 2 | 8 | GET 4, POST 2, PUT 2 | 1 | `GET /strategies/templates` | `80668dce` | 認証なしで呼べるのは 8 中 1 |
...
| `mastodon.local/1.0/openapi.yaml` | 成功 | openapi 3.0 | 127 | DELETE 11, GET 59, PATCH 1, POST 51, PUT 5 | 5 | `GET /api/v1/custom_emojis` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 127 中 5 |
| `mbus.local/0.3.5/openapi.yaml` | 成功 | openapi 3.0 | 7 | GET 2, POST 5 | 2 | `GET /mbus/api` | `80668dce` | 認証なしで呼べるのは 7 中 2 |
| `medium.com/1.0/openapi.yaml` | 成功 | openapi 3.0 | 32 | GET 32 | 1 | `GET /` | `ce61d5ae` | 参照のみ、認証なしで呼べるのは 32 中 1 |
| `meilisearch.com/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 66 | DELETE 17, GET 25, PATCH 6, POST 9, PUT 9 | 24 | `GET /health` | `80668dce` | メソッドが多い、認証なしで呼べるのは 66 中 24 |
| `mermade.org.uk/openapi-converter/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 6 | GET 4, POST 2 | 4 | `GET /badge` | `ce61d5ae` | 認証なしで呼べるのは 6 中 4 |
| `motaword.com/1.0/openapi.yaml` | 成功 | openapi 3.0 | 219 | DELETE 13, GET 105, POST 94, PUT 7 | 3 | `GET /` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 219 中 3 |
| `mozilla.com/kinto/1.22/openapi.yaml` | 成功 | openapi 3.0 | 19 | DELETE 1, GET 16, POST 2 | 6 | `GET /` | `ce61d5ae` | 認証なしで呼べるのは 19 中 6 |
| `nasa.gov/asteroids neows/3.4.0/openapi.yaml` | 成功 | openapi 3.0 | 3 | GET 3 | 1 | `GET /api` | `80668dce` | 参照のみ、小規模、認証なしで呼べるのは 3 中 1、政府機関 |
| `naviplancentral.com/factfinder/v1/swagger.yaml` | 成功 | swagger 2 | 151 | DELETE 22, GET 79, POST 25, PUT 25 | 14 | `GET /api/AccountTypes` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 151 中 14 |
| `naviplancentral.com/plan/v1/swagger.yaml` | 成功 | swagger 2 | 64 | GET 54, POST 10 | 4 | `GET /api/Advisors` | `ce61d5ae` | 認証なしで呼べるのは 64 中 4 |
| `ndhm.gov.in/ndhm-cm/0.5/openapi.yaml` | 成功 | openapi 3.0 | 22 | GET 1, POST 21 | 1 | `GET /v0.5/heartbeat` | `80668dce` | 認証なしで呼べるのは 22 中 1 |
...
| `nebl.io/1.3.0/openapi.yaml` | 成功 | openapi 3.0 | 50 | GET 39, POST 11 | 2 | `GET /ins/sync` | `80668dce` | 認証なしで呼べるのは 50 中 2 |
| `neowsapp.com/1.0/openapi.yaml` | 成功 | openapi 3.0 | 7 | GET 7 | 3 | `GET /rest/v1/neo/browse` | `ce61d5ae` | 参照のみ、認証なしで呼べるのは 7 中 3 |
| `nexmo.com/account/1.0.4/openapi.yaml` | 成功 | openapi 3.0 | 8 | DELETE 1, GET 3, POST 4 | 1 | `GET /account/get-balance` | `80668dce` | 認証なしで呼べるのは 8 中 1 |
| `nexmo.com/media/1.0.2/openapi.yaml` | 成功 | openapi 3.0 | 4 | DELETE 1, GET 2, PUT 1 | 2 | `GET /` | `ce61d5ae` | 認証なしで呼べるのは 4 中 2 |
| `noosh.com/1.0/openapi.yaml` | 成功 | openapi 3.0 | 102 | DELETE 2, GET 77, PATCH 1, POST 14, PUT 8 | 1 | `GET /v1/countries` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 102 中 1 |
...
| `nsidc.org/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 4 | GET 4 | 1 | `GET /OpenSearchDescription` | `80668dce` | 参照のみ、認証なしで呼べるのは 4 中 1 |
...
| `openaq.local/2.0.0/openapi.yaml` | 成功 | openapi 3.0 | 36 | GET 36 | 9 | `GET /favicon.ico` | `80668dce` | 参照のみ、認証なしで呼べるのは 36 中 9 |
...
| `osf.io/2.0/openapi.yaml` | 成功 | openapi 3.0 | 156 | DELETE 10, GET 116, PATCH 10, POST 19, PUT 1 | 16 | `GET /` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 156 中 16 |
...
| `patrowl.local/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 14 | GET 13, POST 1 | 9 | `GET /` | `80668dce` | 認証なしで呼べるのは 14 中 9 |
| `peel-ci.com/1.0.0/swagger.yaml` | 成功 | swagger 2 | 5 | GET 5 | 1 | `GET /health` | `ce61d5ae` | 参照のみ、認証なしで呼べるのは 5 中 1 |
| `pendo.io/1.0.0/swagger.yaml` | 成功 | swagger 2 | 31 | DELETE 5, GET 14, POST 10, PUT 2 | 1 | `GET /health-check/ping` | `80668dce` | メソッドが多い、認証なしで呼べるのは 31 中 1 |
| `peoplegeneratorapi.live/v0/openapi.yaml` | 成功 | openapi 3.0 | 46 | GET 46 | 44 | `GET /api/address` | `80668dce` | 参照のみ、認証なしで呼べるのは 46 中 44 |
...
| `presalytics.io/ooxml/0.1.0/openapi.yaml` | 成功 | openapi 3.0 | 148 | DELETE 1, GET 132, POST 2, PUT 13 | 14 | `GET /Charts/AxisDataTypes` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 148 中 14 |
| `probely.com/1.2.0/openapi.yaml` | 成功 | openapi 3.0 | 143 | DELETE 9, GET 71, PATCH 15, POST 34, PUT 14 | 1 | `GET /plans/` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 143 中 1 |
...
| `reloadly.com/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 2 | GET 1, POST 1 | 1 | `GET /questions` | `80668dce` | 小規模、認証なしで呼べるのは 2 中 1 |
| `reverb.com/3.0/openapi.yaml` | 成功 | openapi 3.0 | 163 | DELETE 17, GET 101, POST 35, PUT 10 | 16 | `GET /articles/categories` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 163 中 16 |
| `ritekit.com/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 13 | GET 13 | 11 | `GET /v1/emoji/auto-emojify` | `ce61d5ae` | 参照のみ、認証なしで呼べるのは 13 中 11 |
| `rumble.run/2.15.0/openapi.yaml` | 成功 | openapi 3.0 | 116 | DELETE 14, GET 68, PATCH 15, POST 7, PUT 12 | 3 | `GET /releases/agent/version` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 116 中 3 |
| `salesloft.com/v2/openapi.yaml` | 成功 | openapi 3.0 | 121 | DELETE 9, GET 74, POST 24, PUT 14 | 2 | `GET /v2/me.json` | `80668dce` | メソッドが多い、大規模、認証なしで呼べるのは 121 中 2 |
| `scrapewebsite.email/0.1/swagger.yaml` | 成功 | swagger 2 | 3 | GET 3 | 1 | `GET /v1/ping.json` | `80668dce` | 参照のみ、小規模、認証なしで呼べるのは 3 中 1 |
| `sheetlabs.com/rig-veda/1.2/swagger.yaml` | 成功 | swagger 2 | 1 | GET 1 | 1 | `GET /resources` | `ce61d5ae` | 参照のみ、小規模 |
| `shipstation.com/1.0.0/openapi.yaml` | 成功 | openapi 3.0 | 2 | GET 1, POST 1 | 1 | `GET /questions` | `80668dce` | 小規模、認証なしで呼べるのは 2 中 1 |
