...
const SPEC_SOURCES = [
    // skipped: no case calls the petstore any more, so there is nothing left
    // to fetch these for
...
const BRUNO_SOURCES = [
    // A hundred odd Bruno collections cloned from public repositories. Like
    // the corpus above they were a breadth check rather than a test service,
    // and no case below calls them, so cloning them only lengthened the run.
...
const CASES = [
    // Only httpbingo is left. It exists to be called, it answers every method
    // and auth scheme the generator emits, and it does not carry state that
    // moves under the comparison. Everything below it is commented out either
    // because it is somebody's real service or because it only ever exercised
    // a GET that the calls here already cover.
    { source: 'httpbingo', method: 'get', path: '/get' },
    { source: 'httpbingo', method: 'post', path: '/post' },
    { source: 'httpbingo', method: 'put', path: '/put' },
    { source: 'httpbingo', method: 'patch', path: '/patch' },
    { source: 'httpbingo', method: 'delete', path: '/delete' },
    ...
    // skipped: this calls the real GitHub API, which is not a test service
    ...
    // skipped: httpbingo covers every one of these shapes above
...
const CORPUS_SPECS = [
    // These are ninety odd real services from the apis.guru directory. They
    // were a breadth check on how widely the generator holds up, but they are
    // not test APIs: they rate limit, go down, and get dropped as unreachable,
    // which says nothing about this repository. The httpbingo cases above
    // cover the call shapes deliberately instead.
